...
export default function ProfileScreen() {
  ...
  const handleThemeChange = async (value: "light" | "dark") => {
    const user = auth.currentUser;
    if (!user) {
      alert("Hata", "Kullanıcı oturumu bulunamadı.");
      return;
    }
    ...
    try {
      ...
      alert("Hata", "Tema kaydedilirken bir sorun oluştu.");
    }
  };
  ...
  const handleImageUpload = async () => {
    try {
      ...
      alert("Başarılı", "Profil fotoğrafınız güncellendi!");
    } catch (error: any) {
  ...
  const handleChangePassword = async () => {
    ...
    if (!oldPassword || !newPassword || !newPasswordAgain) {
      alert("Uyarı", "Lütfen tüm şifre alanlarını doldurun.");
      return;
    }

    if (newPassword !== newPasswordAgain) {
      alert("Uyarı", "Yeni şifre ve tekrarı birbiriyle eşleşmiyor.");
      return;
    }
    ...
    if (newPassword.length < 6) {
      alert("Uyarı", "Yeni şifreniz en az 6 karakter olmalıdır.");
      return;
    }
    ...
    try {
      ...
      alert("Başarılı", "Şifreniz başarıyla güncellendi.");
    } catch (error: any) {
      ...
      if (
        ...
        alert("Hata", "Eski şifrenizi hatalı girdiniz.");
